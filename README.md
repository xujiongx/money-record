...
布布与一二共用的家庭账本：移动端视口、橙粉渐变 UI、多家庭编码隔离、收支记账、成员统计与图表；成员页「更多功能」含**大事记账**（与日常流水隔离）。
...
在 [Supabase](https://supabase.com) 新建项目，打开 **SQL Editor**，按序执行仓库内：

`supabase/migrations/001_init.sql`（及后续 `002`–`004`，含大事记账表）
...
验证或创建成功后，编码会写入 **httpOnly Cookie** 与 **localStorage**。在 **成员** 页底部可进入「更多功能」（大事记账等），或「切换家庭 / 重新输入编码」。
